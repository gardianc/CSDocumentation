...
Las únicas correcciones que se consideran significativas son la corrección por polinomio interpolador del patrón y, en el caso de temperatura de contacto, la corrección por error de inmersión. El resto de las contribuciones se considera despreciable en corrección (tienen estimación nula), aunque aporten en la incertidumbre (ver \cref{sec:componentes}).
...
donde $T_{set}$ es el punto de seteo, la temperatura nominal del punto de calibración, $T_{amb}$ la temperatura ambiente, $K$ una constante menor pero cercana a 1, $L$ la longitud de inmersión y $D_{eff}$ el diámetro efectivo del termómetro \autocite{NW4.4.1,TAVENER}.
Tomando $K \approx 1$ resulta $D_{eff} = D$ para baños líquidos, y $D_{eff} = 2D$ para bloques secos, con $D$ el diámetro del termómetro,. Se puede reescribir la fórmula definiendo una constante $\gamma = \dfrac{D_{eff}}{D}$ = 1 ó 2 según si el medio es baño líquido o bloque seco, de modo que las correcciones resultan
...
Para el caso de mediciones en laboratorio el patrón siempre se inserta a la inmersión máxima posible del medio ($L_{pat}$ = longitud de inmersión máxima del medio). Para la UBP se sobreestima la corrección asumiento que está a la inmersión mínima posible en el medio ($L_{ubp}$ = longitud de inmersión mínima del medio) ya que las longitudes de los sensores de las distintas unidades bajo prueba pueden variar.
...
Fenómeno que produce una diferencia en las lecturas de un instrumento de medición dpendiendo de si el valor fue alcanzado en forma ascendente o descendente. Su distribución de probabilidad de prgien es rectangular de ancho completo. Se aplica uno de dos criterios:
...
donde $T_{max}$ y $T_{min}$ son el mayor y el menor punto de calibración ensayados\cite{DKD8.10}. Al obtenerse de información conocida y no de una serie de mediciones es una incertidumbre de tipo B.

% **[th-001 §4, p.6; DKD-R 5-1 §8.10, p.19; N&W §2.7.3 Ej. 2.10, p.61]**

\textit{Criterio 2}: Se registran mediciones en el punto medio del rango durante las carrera ascendente y descendiente. La histéresis es la diferencia entre los promedios de ambas series\cite{DKD8.10}:
...
donde $a$, $b$ y $c$ son los coeficientes del ajuste cuadrático de la incertidumbre expandida del certificado en función de la temperatura. Tiene distribución normal y al obtenerse de información conocida (los coeficientes del ajuste) y no de una serie de mediciones es de tipo B.
...
Análoga a la interpolación de la corrección, pero sobre el ajuste cuadrático de la componente anterior: es la incertidumbre debido a haber empleado el método de mínimos cuadrados para conocer la incertidumbre de calibración en puntos intermedios.
...
donde $tol_{p,\textit{pat}}$ y $tol_{f,\textit{pat}}$ son la tolerancia porcentual y la tolerancia fija especificadas por el fabricante del patrón. Tiene distribución rectangular y al obtenerse de una especificación es de tipo B.

No debe confundirse con la tolerancia de calibración de la sec. 1.5.3, que la solicita el cliente para cada punto y se evalúa en el punto de seteo $T_{set}$.
...
\begin{table}[H]
\centering
\caption{Tabla de componentes de incertidumbre para calibraciones de temperatura de contacto y ambiental. En violeta se pintan las componentes correspondientes a la UBP, en amarillo las relacionadas al patrón y en rojo las que provienen del medio.}\label{tab:my-table}
\resizebox{\textwidth}{!}{%
...
6 &
  ...
  $\sqrt{\dfrac{\sum \textit{Residuos}^2}{n_{\textit{fit ccr}}-2}}$ &
  Normal &
  1 &
  A &
  $n_{\textit{fit ccr}}-2$ &
...
8 &
  ...
  $\dfrac{1}{2}\sqrt{\dfrac{\sum \textit{Residuos}^2}{n_{\textit{fit cal}}-3}}$ &
  Normal &
  1 &
  A &
  $n_{\textit{fit cal}}-3$ &
...
(1): La componentes 11 y 12 aplican sólo calibraciones de temperatura de contacto realizadas en bloques secos, y la componente 13 a calibraciones de temperatura de contacto realizadas en baños líquidos o calibraciones de temperatura ambiental. Son mutuamente excluyentes.