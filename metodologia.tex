...
En la mayor parte de los casos, un \textit{mensurando} o \textit{magnitud de salida} $Y$ que se busca reportar como resultado de una medición o calibración no se mide directamente, sino que se determina a partir de otras $N$ \textit{magnitudes de entrada} $X_1, X_2, \ldots, X_N$, por medio de una relación funcional $f$ :
...
El resultado de la medición $y$ es una \textit{estimación} del mensurando $Y$, que se obtiene introduciendo las $N$ estimaciones de entrada $x_1, x_2, \ldots, x_N$ para los valores de las $N$ magnitudes $X_1, X_2, \ldots, X_N$ en la ecuación **[]**:
...
Las componentes de incertidumbre se clasifican por su método de evaluación en incertidumbres de tipo A y de tipo B. Las incertidumbres de tipo A son aquellas que se obtienen mediante el análisis estadístico de serie de observaciones. **[GUM 4.2]** El número de grados de libertad para incertidumbres de este tipo es 

\begin{equation}
  \nu = n - m,
\end{equation}

donde n es el número de observaciones y m el número de restricciones involucradas en la estimación. **[GUM 4.2, G.3.3]** Las incertidumbres de tipo B son aquellas que se obtienen por cualquier otro método que no sea el análisis estadístico de observaciones **[GUM 4.3]**. Para las incertidumbres de tipo B el grado de libertad es infinito **[GUM G.6.4]**, aunque en la práctica basta con asumir $\nu \approx 100$.
...
es el \textit{coeficiente de sensibilidad} de la componente, un parámetro que describe cuán sensible es el resultado de la medición a variaciones en la magnitud de entrada $x_i$. Los $c_i$ también pueden calcularse de forma experimental.
...
U define un intervalo $y - U$ a $y + U$ alrededor del resultado de medición que comprende una fracción $p$ de la distribución de probabilidad de salida de $y$ y $u_c(y)$. $p$ es el \textit{nivel de confianza} o \textit{probabilidad} del intervalo, la probabilidad de que el valor ``verdadero'' de la magnitud de salida $Y$ se encuentre dentro de ese intervalo (probabilidad de que $y-U\leq Y \leq y+U$). 
**[GUM 6.2]**
